...
void computeF(const Eigen::Matrix<double,3,1> &f_i, const Eigen::Matrix<double,3,3> &R_body_to_nav_next, Eigen::Matrix<double,15,15> &F)
{
	...
	F.block<3,3>(9,9) = Eigen::Matrix<double,3,3>::Identity()*-1.0/200; // Fa
	F.block<3,3>(12,12) = Eigen::Matrix<double,3,3>::Identity()*-1.0/200; // Fg
...
void computeG(const Eigen::Matrix<double,3,3> &R_body_to_nav_next, Eigen::Matrix<double,15,12> &G)
{
	...
	G.block<3,3>(9,6) = Eigen::Matrix<double,3,3>::Identity();
	G.block<3,3>(12,9) = Eigen::Matrix<double,3,3>::Identity();
	
}


void computePhiAndQdk(const Eigen::Matrix<double,3,1> &f_i, const Eigen::Matrix<double,3,3> &R_body_to_nav_next, Eigen::Matrix<double,15,15> &Phi, Eigen::Matrix<double,15,15> &Qdk)
{
...
void stationaryMeasurementUpdate(const Eigen::Matrix<double,3,3> & R_body_to_nav)
{
	...
	Eigen::Matrix<double,3,15> Ha = Eigen::Matrix<double,3,15>::Zero();
...
void encoderMeasurementUpdate()
{
	...
	Eigen::Matrix<double,3,1> p_meas = enc_meas(Eigen::seq(0,2));
...
void EKF(const Eigen::MatrixXd & H, const Eigen::MatrixXd & R, const Eigen::MatrixXd & z, const bool update_bias)
{
	...
	// 	rotation update 
	Eigen::Matrix<double,3,1> ro = dx(Eigen::seq(6,8));

	Eigen::Matrix<double,3,3> P(3,3);
	to_skew(ro,P);
	// predicted orientation
	Eigen::Quaternion<double> b_q = Eigen::Quaternion<double>(state[6],state[7],state[8],state[9]);
...
{		
	...
	double Dl = (ticks_l_curr-ticks_l_prev)/filter.ticks_per_m;
	double Dr = (ticks_r_curr-ticks_r_prev)/filter.ticks_per_m;
	double Dc = (Dl+Dr)/2.0;
	...
	Eigen::Matrix<double,3,1> pos(enc_meas[0],enc_meas[1],enc_meas[2]);

	// get encoder model orientation
	Eigen::Quaternion<double> b_q = Eigen::Quaternion<double>(enc_meas[4],enc_meas[5],enc_meas[6],enc_meas[7]);

	// b is nav to body
	Eigen::Quaternion<double> b_inv = b_q.inverse();
	...
	Eigen::Matrix<double,3,3> Q_tst;
	double sigma_squared = 0.25*filter.k*filter.k*(Dl*Dl+Dr*Dr);
	double sigma_squared_diff = 0.25*filter.k*filter.k*(std::abs(Dl*Dl-Dr*Dr));


	Q_tst << sigma_squared, 0.0, 0.0, 0.0, sigma_squared + sigma_squared_diff, 0.0, 0.0,0.0,sigma_squared*0.0001; 
	...
	Eigen::Matrix<double,3,3> R_body_to_nav = b_inv.toRotationMatrix();
	...
	if (false)
	{
		Eigen::Quaternion<double> b_next_body_to_nav = b_inv;
...
void imu_callback(const sensor_msgs::Imu::ConstPtr& msg)
{
	...
	// current position
	Eigen::Matrix<double,3,1> p = state(Eigen::seq(0,2));

	// current velocity
	Eigen::Matrix<double,3,1> v = state(Eigen::seq(3,5));

	// current orientation
	Eigen::Matrix<double,4,1> b = state(Eigen::seq(6,9));
	Eigen::Quaternion<double> b_prev = Eigen::Quaternion<double>(b(0),b(1),b(2),b(3)); // w, x, y, z //could also be called b_current

	// current accel bias
	Eigen::Matrix<double,3,1> x_a = state(Eigen::seq(10,12));

	// current gyro bias
	Eigen::Matrix<double,3,1> x_g = state(Eigen::seq(13,15));
	...
	Eigen::Matrix<double,15,15> Phi(15,15);
	// compute Qdk (discrete noise). Qdk is 15 x 15
	Eigen::Matrix<double,15,15> Qdk(15,15);
...
void initialize_ekf(ros::NodeHandle &n)
{
	...
	if (client.call(srv)) //see handle_init_ekf
	{
		...
		n.param<double>("k",filter.k,0.4); //TODO  keep editing and discover better definition of this 
					//note that the slip can only cause you to overshoot your estimate

		// robot dimensional parameters
		n.param<double>("L",filter.L, 0.577); // base width (m)
		n.param<double>("R",filter.R, 0.1016); // wheel radius (m)
		n.param<int>("ticks_per_rev", filter.ticks_per_rev, 1316); // wheel encoder parameters
		n.param<double>("ticks_per_m", filter.ticks_per_m,filter.ticks_per_rev/(PI*2*filter.R));

		//http://web.mit.edu/2.05/www/Handout/HO2.PDF, euler angles != RPY. xyz euler = zyx rpy
		// Eigen::Quaternion<double> b_q = Eigen::Quaternion<double>(b.w,b.x,b.y,b.z);
		// Eigen::Matrix<double,3,1> euler_angles = b_q.inverse().toRotationMatrix().eulerAngles(0,1,2); // 'xyz'
		// double yaw_init = euler_angles[2];
		...
		cov.block<2,2>(6,6) = (sigma_nua/filter.g)*(sigma_nua/filter.g)/T*Eigen::Matrix<double,2,2>::Identity(); //orientation uncertainty
		cov.block<3,3>(12,12) = (sigma_nug)*(sigma_nug)/T*Eigen::Matrix<double,3,3>::Identity(); //gyro bias uncertainty
